...
由依赖图可知，ToolRegistry 与 AgentLoop 之间构成双向关联：一方面，AgentLoop 依赖 ToolRegistry 完成工具调用；另一方面，ToolRegistry 向 AgentLoop 提供可用工具集合及其接口描述。由此可见，ToolRegistry 是 Agent 执行外部动作能力的核心基础设施。
